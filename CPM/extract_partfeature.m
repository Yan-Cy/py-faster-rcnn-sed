function features = extract_partfeature(test_image, param, rectangle, net, interest_layers, parts)
tic;
...
for m = 1:length(multiplier)
    ...
    [imageToTest, pad{m}, centerp] = padAround(imageToTest, boxsize, center_s, model.padValue); % into boxsize, which is multipler of 4
    
    imageToTest = preprocess(imageToTest, 0.5, param);
     
    feature = get_feature(imageToTest, net, interest_layers);
    
    nfilter = length(feature{1}(1,1,:));
    for i = 1:length(parts)
        t = zeros(1, nfilter);
        if parts(i,1) ~= 0 || parts(i,2) ~= 0
            pose = parts(i) * scale +  [max(0,pad{m}(2)) max(0,pad{m}(1))];
            px = ceil((pose(1) - (centerp(1)-(boxsize/2-1))) * 1.0 / boxsize * 46);
            px = min(max(px, 1), 46);
            py = ceil((pose(2) - (centerp(2)-(boxsize/2-1))) * 1.0 / boxsize * 46);
            py = min(max(py, 1), 46);
            t = reshape(feature{1}(py,px,:), 1, []);
        end
        features = [features, t];
    end
end
...
function [img_padded, pad, centerp] = padAround(img, boxsize, center, padValue)
    center = round(center);
    h = size(img, 1);
    w = size(img, 2);
