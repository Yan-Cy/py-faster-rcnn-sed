...
def sed_ap(rec, prec, use_07_metric=False):
...
def sed_eval(detpath,
    ...
    # CLEAN Hard Negative file
    hn_path = os.path.join(annopath, 'hn_roi')
    if os.path.exists(hn_path):
        shutil.rmtree(hn_path)
    os.mkdir(hn_path)
    total_hn = 0
    ...
    for d in range(nd):
        ...
        if ovmax > ovthresh:
            ...
            if confidence[d] > 0.7:
                total_hn = total_hn + 1
                hn_file = os.path.join(hn_path, image_ids[d] + '.roi')
                with open(hn_file, 'a') as f:
                    f.write(str(int(bb[0])) + ' ' + str(int(bb[1])) + ' ' + str(int(bb[2])) + ' ' + str(int(bb[3])) + '\n')
